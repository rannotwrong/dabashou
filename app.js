import { SKILLS, TIMES, LEVELS, people } from './data.js?v=plans-8';
import { STORAGE_KEY, clone, uid, freshState, validState, migrate, combinations, matchPeople, diagnose, LABELS, activeExchange, validPlan, proposalError, transition } from './core.js?v=plans-8';
...
function matchCard(m) {
  ...
  return `<article class="match-card"><div class="card-person"><div class="who"><span class="avatar" aria-hidden="true">${esc(p.name.slice(-1))}</span><strong>${esc(p.name)}</strong><span class="small muted">技能互换</span></div><span class="status neutral">${m.common.length?'时段有交集':'时间待协商'}</span></div><div class="skill-pair"><div><p class="eyebrow">对方能教 · 你想学</p><h3>${esc(pair.want)}</h3><p class="skill-description">${esc(skill.scope)}</p></div><span class="exchange-symbol" aria-hidden="true">⇄</span><div><p class="eyebrow">你能教 · 对方想学</p><h3>${gives.map(esc).join('<span class="or"> 或 </span>')}</h3><p class="skill-description">${gives.length>1?'发起时选择其中一项即可':esc(p.learn.find(s=>s.name===gives[0]).goal)}</p></div></div><div class="match-meta"><span>${esc(m.common.length?m.common.join('、'):p.times.join('、'))}${m.common.length?' · 具体时间待确认':''}</span><span>${p.record?`完成 ${p.record.completed} 次 · 示例记录`:'新伙伴 · 暂无互换记录'}</span></div><div class="card-actions"><div>${button('查看档案','profile',p.id,'quiet')}${button(state.saved.includes(p.id)?'已收藏':'收藏','save',p.id,'quiet',`aria-pressed="${state.saved.includes(p.id)}"`)}</div>${inProgress?button('查看互换','detail',inProgress.id,'primary'):button('查看方案','invite',p.id,'primary')}</div><div class="skip-row">${button('暂不考虑','skip',m.skipKey,'quiet small')}</div></article>`;
}
function directionalCard(m) {
  const p=m.person, learning=m.kind==='learn', entries=learning?m.learning:m.teaching;
  const content=entries.map(s=>`<div><p class="eyebrow">${learning?'付费学技能 · 对方一对一教你':'付费学习需求 · 你一对一教对方'}</p><h3>${esc(s.name)}</h3><p class="skill-description">${esc(learning?s.scope:s.goal)}</p><p class="small muted">${learning?'适合':'学员基础'}：${esc(s.level||'待确认')}</p><div class="price-line"><strong>${learning?'¥':'预算 ≤ ¥'}${esc(learning?s.price:s.budget)}</strong><span> / 次 · ${esc(s.minutes)} 分钟</span></div></div>`).join('');
  return `<article class="match-card"><div class="card-person"><div class="who"><span class="avatar" aria-hidden="true">${esc(p.name.slice(-1))}</span><strong>${esc(p.name)}</strong><span class="small muted">${learning?'技能学习 · 付费':'技能教授 · 收入机会'}</span></div><span class="status neutral">线上 1 对 1</span></div><div class="directional-skills">${content}</div><div class="match-meta"><span>${esc((m.common.length?m.common:p.times).join('、'))} · ${m.common.length?'时段有交集':'时间待协商'}</span><span>${learning?'按次付费，无需互教；价格为示例报价':'学员愿意付费；最终课时与报酬由双方确认，预算为示例'}</span></div><div class="card-actions"><div>${button('查看档案','profile',p.id,'quiet')}${button(state.saved.includes(p.id)?'已收藏':'收藏','save',p.id,'quiet',`aria-pressed="${state.saved.includes(p.id)}"`)}</div>${button('查看方案','paid-enquiry',`${m.kind}:${p.id}`,'primary')}</div><div class="skip-row">${button('暂不考虑','skip',m.skipKey,'quiet small')}</div></article>`;
}
function paidEnquiry(kind,id,existingId='') {
  const p=personBy(id),learning=kind==='learn',existing=state.enquiries.find(e=>e.id===existingId);
  const m=matchPeople({...state,filter:state.filter==='all'?'all':kind,skipped:[]}).find(m=>m.person.id===id&&m.kind===kind);
  const entries=existing?(learning?p.teach:p.learn).filter(s=>s.name===existing.skill):(m?(learning?m.learning:m.teaching):[]);
  if(!entries.length){toast('当前条件没有对应的付费机会，请重新选择。');return;}
  open(learning?'一对一学习方案':'一对一教学方案',`<form id="paid-form" class="form" novalidate><p class="muted">${learning?'向':'为'}${esc(p.name)}${learning?'付费学习':'提供收费教学'} · 线上 1 对 1</p><label>技能<select name="skill">${entries.map(s=>`<option>${esc(s.name)}</option>`).join('')}</select></label><div id="paid-terms" class="goal-reference"></div>${learning?'':`<label>我的教学报价（元／次）<input type="number" name="quote" min="1" max="100000" step="0.01" required></label>`}<label>${learning?'我的学习需求':'我能提供的教学内容'}<textarea name="note" maxlength="200" required></textarea></label><label>期望上课时间<input type="datetime-local" name="when" value="${esc(existing?.when||'')}"></label><label class="check"><input name="flexible" type="checkbox" ${existing?.when?'':'checked'}>还没有确定时间，先协商</label><p class="small muted">课时按上方单次时长约定。费用、时间和教学范围可在聊天中协商，修改方案后仍需双方确认。演示聊天仅在此浏览器保存，不扣款、不结算。</p><p id="form-error" class="form-error" role="alert"></p><div class="form-actions"><button type="submit" class="primary">和TA聊聊</button></div></form>`);
  const form=$('#paid-form');
  const update=()=>{const t=entries.find(s=>s.name===form.elements.skill.value);$('#paid-terms').innerHTML=`<p>${learning?'课程报价':'学员预算上限'}：¥${learning?t.price:t.budget}／次 · ${t.minutes} 分钟</p><p>${esc(learning?t.scope:t.goal)}</p>`;form.elements.note.value=existing?.note||(learning?(state.profile.learn.find(x=>x.name===t.name)?.goal||''):(state.profile.teach.find(x=>x.name===t.name)?.scope||''));if(!learning)form.elements.quote.value=existing?.amount||t.budget;};update();form.elements.skill.onchange=update;
  form.onsubmit=e=>{e.preventDefault();const t=entries.find(s=>s.name===form.elements.skill.value),note=form.elements.note.value.trim(),amount=learning?t.price:Number(form.elements.quote.value),when=form.elements.when.value,flexible=form.elements.flexible.checked;
    const error=!note?'请简要填写需求或教学内容。':(!Number.isFinite(amount)||amount<1||amount>100000)?'请输入 1 至 100000 元的有效报价。':(!when&&!flexible)?'请选择时间，或勾选先协商。':when&&new Date(when).getTime()<=Date.now()?'请选择未来的上课时间。':'';
    if(error){$('#form-error').textContent=error;return;}
    const record={...(existing||{}),id:existing?.id||uid(),kind,personId:id,name:p.name,skill:t.name,amount,minutes:t.minutes,note,when,flexible,status:'待沟通'};
    if(existing)Object.assign(existing,record);else state.enquiries.unshift(record);
    recordPlanMessage(existing||record,'paid');persist();close();route('mine');chat('paid',record.id);
  };
}
function recordPlanMessage(record,type) {
  if(!Array.isArray(record.messages))record.messages=[];
  record.messages.push({role:'plan',text:type==='paid'?`${record.kind==='learn'?'付费学习':'收费教学'} · ${record.skill}\n¥${record.amount}／次 · ${record.minutes} 分钟\n${record.when||'时间待协商'}\n${record.note}`:`技能互换 · 我学 ${record.want}，我教 ${record.give}\n${record.plan.when||'时间待协商'} · 我教 ${record.plan.myMinutes} 分钟，对方教 ${record.plan.theirMinutes} 分钟`,at:Date.now()});
}
function chat(type,id) {
  const record=(type==='paid'?state.enquiries:state.exchanges).find(e=>e.id===id);if(!record)return;
  if(!Array.isArray(record.messages)||!record.messages.length){recordPlanMessage(record,type);persist();}
  open(`和${record.name}聊聊`,`<div class="chat"><p class="small muted">本地演示聊天 · 不发送真实消息。聊天不代表接受方案、付款或成交。</p><div class="section-label"><strong>${esc(type==='paid'?record.skill:record.want+' ⇄ '+record.give)}</strong>${button('修改方案','chat-edit',`${type}:${id}`,'quiet')}</div><div class="chat-messages" role="log" aria-label="协商消息">${record.messages.map(m=>`<div class="chat-message ${m.role==='me'?'mine':m.role==='plan'?'plan':''}"><span class="small muted">${m.role==='plan'?'方案 · 待协商':m.role==='me'?'我':esc(record.name)+'（模拟）'}</span><p>${esc(m.text)}</p></div>`).join('')}</div><form id="chat-form" class="form"><label>协商内容<textarea name="message" maxlength="1000" placeholder="聊聊时间、教学内容或费用…" required></textarea></label><div class="form-actions"><button type="submit" class="primary">发送</button>${button('模拟对方回复','chat-reply',`${type}:${id}`,'secondary')}</div></form></div>`);
  const log=$('.chat-messages');log.scrollTop=log.scrollHeight;
  $('#chat-form').onsubmit=e=>{e.preventDefault();const message=e.target.elements.message.value.trim();if(!message)return;record.messages.push({role:'me',text:message,at:Date.now()});persist();chat(type,id);};
}
function enquiryList() {
  return `<section class="settings-section"><h2>付费学习与教学意向</h2><p class="small muted">本地演示记录 · 不扣款、不结算收入</p>${state.enquiries.length?state.enquiries.map(e=>`<article class="saved-row"><div><strong>${esc(e.skill)} · ${e.kind==='learn'?'课程咨询':'教学报价'}</strong><p>${esc(e.name)} · ¥${esc(e.amount)}／次 · ${esc(e.minutes)} 分钟</p><p class="small muted">${esc(e.note)}</p><span class="small muted">${esc(e.status)} · 尚未成交</span></div>${button('和TA聊聊','chat',`paid:${e.id}`,'secondary')}</article>`).join(''):'<p class="muted">暂无意向，可从发现页咨询课程或提交教学报价。</p>'}</section>`;
}
function emptyHTML() {
...
function renderMine() {
  ...
  $('#app').innerHTML=`<div class="page"><div class="page-heading"><h1>我的技能</h1><p class="muted">能教与想学分别管理，系统为你组合交换方案。</p></div><div class="my-profile"><div class="section-label"><div class="who"><span class="avatar large">我</span><div><h2>我的介绍</h2><p class="muted">${esc(p.bio)}</p></div></div>${button('编辑','bio','','quiet')}</div></div><div class="skill-columns">${['teach','learn'].map(type=>`<section><div class="section-label"><h2>${type==='teach'?'我能教的':'我想学的'} <span class="count">${p[type].length}</span></h2>${button('＋ 添加','add-skill',type,'quiet')}</div>${p[type].map((s,i)=>`<article class="skill-item"><div class="section-label"><h3>${esc(s.name)}</h3>${button('编辑','edit-skill',`${type}:${i}`,'quiet')}</div><p class="small muted">${type==='teach'?'适合':'当前水平'}：${esc(s.level||'待补充')}</p><p>${esc(type==='teach'?(s.scope||'补充可以教的内容，让伙伴更了解你。'):(s.goal||'可以补充一个学习目标作为参考。'))}</p>${type==='teach'&&s.evidence?`<p class="evidence-summary small">能力参考：${esc(s.evidence)}</p>`:''}</article>`).join('')||`<p class="muted">还没有添加技能。</p>`}</section>`).join('')}</div><section class="settings-section"><div class="section-label"><div><h2>可用时间</h2><p>${esc(p.times.join('、')||'尚未设置')} · ${p.flexible?'可协商':'仅这些时段'}</p></div>${button('调整时间','times','','secondary')}</div></section><section class="settings-section"><h2>我的互换记录</h2><p class="muted">${state.exchanges.filter(e=>e.status==='completed').length?`已完成 ${state.exchanges.filter(e=>e.status==='completed').length} 次演示互换。`:'暂无完成记录，完成首次互换后会在这里留下记录。'}</p>${button('查看互换','page','exchanges','quiet')}</section>${enquiryList()}${state.subscriptions.length?`<section class="settings-section"><h2>已订阅的学习意向</h2><p class="small muted">仅在此浏览器保存；演示版不发送通知。</p><div class="subscription-list">${state.subscriptions.map(s=>`<div><span>${esc(s)}</span>${button('取消订阅','unsubscribe',s,'quiet')}</div>`).join('')}</div></section>`:''}</div>`;
}
...
function nextStep(e) {if(['pending','negotiating'].includes(e.status)&&!validPlan(e.plan))return '下一步：在聊天中协商具体时间，修改方案后再由双方确认。';return {pending:'下一步：等待对方接受或提出新方案。',negotiating:'下一步：确认对方提出的新时间与投入。',scheduled:'双方已确认方案，可以开始互换。',learning:'双方分别完成教学后，再确认学习成果。',verifying:'双方分别确认自己的学习结果。',completed:e.review?'评价已保存，可以再次互换。':'互换已结束，可以留下评价或再次互换。',cancelled:'原方案、已完成教学和取消原因均已保留。'}[e.status];}
...
function proposal(id,actor) {
  ...
  open(actor==='them'?'模拟对方提出新方案':'修改交换方案',`<form id="proposal-form" class="form" novalidate><p class="muted">新方案会替换原约定，需另一方再次确认。</p>${planFields(e.plan,true)}<p id="form-error" class="form-error" role="alert"></p><div class="form-actions"><button class="primary" type="submit">${actor==='them'?'模拟提出方案':'发送新方案'}</button></div></form>`);
  $('#proposal-form').onsubmit=event=>{event.preventDefault();const plan=readPlan(event.target);const error=proposalError(plan);if(error){$('#form-error').textContent=error;return;}try{const next=transition(e,actor==='them'?'counter':'propose',plan);Object.assign(e,next);recordPlanMessage(e,'exchange');persist();close();render();toast('新方案已提交，等待另一方确认。');}catch(error){$('#form-error').textContent=error.message;}};
}
...
document.addEventListener('click',event=>{
  ...
  switch(action) {
    ...
    case 'skip':{const match=matchPeople(state).find(m=>m.skipKey===id);if(match)state.skipped.push(match.skipKey);persist();render();toast('已暂时跳过，可在无推荐时重新查看。');break;}
    ...
    case 'chat':{const [type,key]=id.split(':');chat(type,key);break;}
    case 'chat-edit':{const [type,key]=id.split(':');if(type==='paid'){const e=state.enquiries.find(e=>e.id===key);paidEnquiry(e.kind,e.personId,key);}else proposal(key,'me');break;}
    case 'chat-reply':{const [type,key]=id.split(':');const e=(type==='paid'?state.enquiries:state.exchanges).find(e=>e.id===key);e.messages.push({role:'them',text:'收到方案了，我们可以再确认一下上课时间和本次教学范围。'+(type==='paid'?'费用也需要双方确认。':''),at:Date.now()});persist();chat(type,key);break;}
    case 'paid-enquiry':{const [kind,person]=id.split(':');paidEnquiry(kind,person);break;}
