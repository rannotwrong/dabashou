import { SKILLS, TIMES, LEVELS, people } from './data.js?v=paid-6';
import { STORAGE_KEY, clone, uid, freshState, validState, migrate, combinations, matchPeople, diagnose, LABELS, activeExchange, validPlan, transition } from './core.js?v=paid-6';
...
function directionalCard(m) {
  ...
  return `<article class="match-card"><div class="card-person"><div class="who"><span class="avatar" aria-hidden="true">${esc(p.name.slice(-1))}</span><strong>${esc(p.name)}</strong><span class="small muted">${learning?'技能学习 · 付费':'技能教授 · 收入机会'}</span></div><span class="status neutral">线上 1 对 1</span></div><div class="directional-skills">${content}</div><div class="match-meta"><span>${esc((m.common.length?m.common:p.times).join('、'))} · ${m.common.length?'时段有交集':'时间待协商'}</span><span>${learning?'按次付费，无需互教；价格为示例报价':'学员愿意付费；最终课时与报酬由双方确认，预算为示例'}</span></div><div class="card-actions"><div>${button('查看档案','profile',p.id,'quiet')}${button(state.saved.includes(p.id)?'已收藏':'收藏','save',p.id,'quiet',`aria-pressed="${state.saved.includes(p.id)}"`)}</div>${button(learning?'咨询课程':'提交教学报价','paid-enquiry',`${m.kind}:${p.id}`,'primary')}</div><div class="skip-row">${button('暂不考虑','skip',m.skipKey,'quiet small')}</div></article>`;
}
function paidEnquiry(kind,id) {
  const p=personBy(id),learning=kind==='learn';
  const m=matchPeople({...state,filter:state.filter==='all'?'all':kind,skipped:[]}).find(m=>m.person.id===id&&m.kind===kind);
  if(!m){toast('当前条件没有对应的付费机会，请重新选择。');return;}
  const entries=learning?m.learning:m.teaching;
  open(learning?'咨询一对一课程':'提交一对一教学报价',`<form id="paid-form" class="form"><p class="muted">${learning?'向':'为'}${esc(p.name)}${learning?'咨询付费课程':'提供收费教学'} · 单向教学，无需互换技能</p><label>技能<select name="skill">${entries.map(s=>`<option>${esc(s.name)}</option>`).join('')}</select></label><div id="paid-terms" class="goal-reference"></div>${learning?'':`<label>我的教学报价（元／次）<input type="number" name="quote" min="1" max="100000" step="0.01" required></label>`}<label>${learning?'我的学习需求':'我能提供的教学内容'}<textarea name="note" maxlength="200" required></textarea></label><p class="small muted">仅保存本地演示意向，不发送真实消息、不扣款，也不产生收入。费用和预约尚未确认。</p><p id="form-error" class="form-error" role="alert"></p><div class="form-actions"><button type="submit" class="primary">${learning?'保存课程咨询':'保存教学报价'}</button></div></form>`);
  const form=$('#paid-form');
  const update=()=>{const s=entries.find(s=>s.name===form.elements.skill.value);$('#paid-terms').innerHTML=`<p>${learning?'课程报价':'学员预算上限'}：¥${learning?s.price:s.budget}／次 · ${s.minutes} 分钟</p><p>${esc(learning?s.scope:s.goal)}</p>`;form.elements.note.value=learning?(state.profile.learn.find(x=>x.name===s.name)?.goal||''):(state.profile.teach.find(x=>x.name===s.name)?.scope||'');if(!learning)form.elements.quote.value=s.budget;};update();form.elements.skill.onchange=update;
  form.onsubmit=e=>{e.preventDefault();const s=entries.find(s=>s.name===form.elements.skill.value),note=form.elements.note.value.trim(),amount=learning?s.price:Number(form.elements.quote.value);if(!note){$('#form-error').textContent='请简要填写需求或教学内容。';return;}if(!Number.isFinite(amount)||amount<=0){$('#form-error').textContent='请输入有效报价。';return;}if(!learning&&amount>s.budget){$('#form-error').textContent='报价超过学员预算，请调整报价后提交。';return;}state.enquiries.unshift({id:uid(),kind,personId:id,name:p.name,skill:s.name,amount,minutes:s.minutes,note,status:'待沟通'});persist();close();route('mine');toast(learning?'课程咨询已保存，尚未预约或付款。':'教学报价已保存，尚未成交或产生收入。');};
}
function enquiryList() {
  return `<section class="settings-section"><h2>付费学习与教学意向</h2><p class="small muted">本地演示记录 · 不扣款、不结算收入</p>${state.enquiries.length?state.enquiries.map(e=>`<article class="saved-row"><div><strong>${esc(e.skill)} · ${e.kind==='learn'?'课程咨询':'教学报价'}</strong><p>${esc(e.name)} · ¥${esc(e.amount)}／次 · ${esc(e.minutes)} 分钟</p><p class="small muted">${esc(e.note)}</p><span class="small muted">${esc(e.status)} · 尚未成交</span></div></article>`).join(''):'<p class="muted">暂无意向，可从发现页咨询课程或提交教学报价。</p>'}</section>`;
}
function emptyHTML() {
